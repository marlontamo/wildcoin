...
#if defined(NDEBUG)
# error "wildcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd6e79ffac41bd09d0537634e2db8b5de5a466f3a67a9239f3408a39cd5a3d397");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // wildcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "wildcoin Signed Message:\n";
...
{
    // wildcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // wildcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // wildcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // wildcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // wildcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // wildcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd3250b91efd1ffc02d169dcaf317d2cce309f46a69b6d45c7c6a0629c0800cac");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "5/16/2016  Most Memorable date for me :)";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a22a6e6e9acc5685adb39434b9fd4d153f82f3068dab8afa68b7e8cb64e5e226165d8c9f12a1f44bca0ac80c8d22404d153c68a49e7b261cb01bc17212623232") << OP_CHECKSIG;
        ...
        block.nVersion = 1;
        block.nTime    = 1585225253;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084698635;

        if (fTestNet)
        {
            block.nTime    = 1585225224;
            block.nNonce   = 386545615;
        }
        //start statement
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x38b83b7b8ddea4c29a2d5ac0610db5f7c8d5567029b780e3586e4e62c0fe8f20"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc4, 0xb8, 0xda }; // wildcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("wildcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("wildcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static wildcoinMiner(CWallet *pwallet)
{
    printf("wildcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wildcoin-miner");
    ...
    try { loop {
        ...
        printf("Running wildcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("wildcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&wildcoinMiner, pwallet));
}
